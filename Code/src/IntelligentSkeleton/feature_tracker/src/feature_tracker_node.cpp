...
ros::Publisher pub_rgb_raw_cloud;
...
void img_callback(const sensor_msgs::ImageConstPtr &color_msg, const sensor_msgs::ImageConstPtr &depth_msg)
{
    ...
    // cv::Mat color_map = color_ptr->image;
    ...
    // cv::bilateralFilter(depth_ptr->image, depth_ptr->image, 9, 11, 15);
...
//always 0
    ...
    if (PUB_THIS_FRAME)
    {
        ...
        if (SHOW_TRACK)
        {
            ...
            // cv::Mat depth_map = depth_ptr->image;
            ...
            // // rgb_cloud->reserve(color_map.cols * color_map.rows);
            // for(int i = 0; i < color_map.rows; i++) {
            //     for(int j = 0; j < color_map.cols; j++) {
            //         auto color = color_map.at<cv::Vec3b>(i, j);
            //         auto depth = (float)depth_map.at<unsigned short>(i, j) / 1000.0f;

            //         if(depth <= 0.05f || depth >= 6.0f) continue;

            //         float px = (j - CX) / FX * depth;
            //         float py = (i - CY) / FY * depth;
            //         float pz = depth;
            ...
            // pub_rgb_raw_cloud.publish(cloud_msg);
        }
    }
...
int main(int argc, char **argv)
{
    ...
    //std::cout << typeid(sub_image).name() << std::endl;
    std::cout << "-----------------------------------------DEPTH_TOPIC" << std::endl;
    std::cout << DEPTH_TOPIC << std::endl;
...
//    message_filters::TimeSynchronizer<sensor_msgs::Image, sensor_msgs::Image> sync(sub_image, sub_depth, 100);
    ...
    pub_rgb_raw_cloud = n.advertise<sensor_msgs::PointCloud2>("raw_rgb_cloud", 1000);
    /*
    if (SHOW_TRACK)
