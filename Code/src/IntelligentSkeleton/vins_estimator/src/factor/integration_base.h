...
class IntegrationBase
{
  public:
    ...
    Eigen::Quaterniond getDeltaRotation(const Eigen::Vector3d& Bg)
    {
        Eigen::Matrix3d dr_dbg = jacobian.block<3, 3>(O_R, O_BG);
        Eigen::Vector3d dbg = Bg - linearized_bg;

        return (delta_q * Utility::deltaQ(dr_dbg * dbg)).normalized();
    }
    ...
    Eigen::Vector3d getDeltaPosition(const Eigen::Vector3d& Ba, const Eigen::Vector3d& Bg)
    {
        ...
        Eigen::Vector3d dba = Ba - linearized_ba;
        Eigen::Vector3d dbg = Bg - linearized_bg;

        return delta_p + dp_dba * dba + dp_dbg * dbg;
    }
    ...
    Eigen::Vector3d getDeltaVelocity(const Eigen::Vector3d& Ba, const Eigen::Vector3d& Bg)
    {
        ...
        return delta_v + dv_dba * dba + dv_dbg * dbg;
    }
