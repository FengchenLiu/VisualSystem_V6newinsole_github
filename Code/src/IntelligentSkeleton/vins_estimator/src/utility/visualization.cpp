...
ros::Publisher pub_oldest_camera_pose;
...
{
    pub_latest_odometry = n.advertise<nav_msgs::Odometry>("imu_propagate", 1000); //imu-rate pose (last time)
    pub_path = n.advertise<nav_msgs::Path>("path", 1000);// set of newest sliding window imu pose
    pub_relo_path = n.advertise<nav_msgs::Path>("relocalization_path", 1000);
    pub_odometry = n.advertise<nav_msgs::Odometry>("odometry", 1000); // newest sliding window imu pose
    pub_point_cloud = n.advertise<sensor_msgs::PointCloud>("point_cloud", 1000); // iterate estimator.f_manager.feature
    pub_margin_cloud = n.advertise<sensor_msgs::PointCloud>("history_cloud", 1000);
    pub_key_poses = n.advertise<visualization_msgs::Marker>("key_poses", 1000); // sliding window all xyz pose
    pub_camera_pose = n.advertise<nav_msgs::Odometry>("camera_pose", 1000); // WINDOW_SIZE - 1 camera pose
    pub_oldest_camera_pose = n.advertise<nav_msgs::Odometry>("oldest_camera_pose", 1000);
    pub_camera_pose_visual = n.advertise<visualization_msgs::MarkerArray>("camera_pose_visual", 1000);
    pub_keyframe_pose = n.advertise<nav_msgs::Odometry>("keyframe_pose", 1000); // WINDOW_SIZE - 2 imu pose
    pub_keyframe_point = n.advertise<sensor_msgs::PointCloud>("keyframe_point", 1000);
    pub_extrinsic = n.advertise<nav_msgs::Odometry>("extrinsic", 1000);
    pub_relo_relative_pose=  n.advertise<nav_msgs::Odometry>("relo_relative_pose", 1000);
    pub_plane_seg = n.advertise<bigai_robotic_msgs::PlaneSegVec>("plane_seg", 1000);
...
void pubKeyPoses(const Estimator &estimator, const std_msgs::Header &header)
{
    ...
    for (int i = 0; i <= WINDOW_SIZE; i++)
    {
        ...
        correct_pose = estimator.key_poses[i]; // sliding window all
...
void pubOldestCameraPose(const Estimator &estimator, const std_msgs::Header &header)
{

    if (estimator.solver_flag == Estimator::SolverFlag::NON_LINEAR)
    {
        Vector3d P = estimator.Ps[0] + estimator.Rs[0] * estimator.tic[0];
        Quaterniond R = Quaterniond(estimator.Rs[0] * estimator.ric[0]);
        ...
        odometry.pose.pose.orientation.w = R.w();
        //"camera_pose"
        pub_oldest_camera_pose.publish(odometry);
...
{
    sensor_msgs::PointCloud point_cloud, loop_closure_point_cloud;
    point_cloud.header = header;
    loop_closure_point_cloud.header = header;
    ...
    for (auto &it_per_id : estimator.f_manager.feature)
    {
        ...
        double depth = it_per_id.feature_per_frame[0].measure_depth;
        //camera coordinate
...
void pubPlaneSeg(const std::map<ushort, PlaneInfo::Ptr>& all_planes, const std_msgs::Header header)
{
    bigai_robotic_msgs::PlaneSegVec plane_seg_msg;
    plane_seg_msg.header = header;
}