...
public:
  FeaturePerFrame(const Eigen::Matrix<double, 8, 1> &_point, double td)
  {
      ...
      velocity.x() = _point(5);
      velocity.y() = _point(6);
      measure_depth = _point(7);
      cur_td = td;
      valid_project_edge = true;
  }
  double cur_td;
  Vector3d point;
  Vector2d uv;
...
public:
  ...
  bool valid_host_edge;
  int valid_project_num;

  Vector3d gt_p;
  Vector3f color;

  FeaturePerId(int _feature_id, int _start_frame, double _host_depth)
      : feature_id(_feature_id), start_frame(_start_frame),
        used_num(0), estimated_depth(-1.0), host_depth(_host_depth), estimate_flag(0), solve_flag(0),
        valid_host_edge(true), valid_project_num(0) 
  {
    color = Eigen::Vector3f::Random();
  }
