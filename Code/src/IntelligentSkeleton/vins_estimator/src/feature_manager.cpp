...
vector<pair<Vector3d, Vector3d>> FeatureManager::getCorrespondingWithDepth(int frame_count_l, int frame_count_r)
{
	...
	for (auto &it : feature)
	{
		if (it.start_frame <= frame_count_l && it.endFrame() >= frame_count_r)
		{
			...
			double depth_a = it.feature_per_frame[idx_l].measure_depth;
			if (depth_a < 0.1 || depth_a > 10)//max and min measurement
			    continue;
			double depth_b = it.feature_per_frame[idx_r].measure_depth;
...
void FeatureManager::triangulate(Vector3d Ps[], Vector3d tic[], Matrix3d ric[])
{
    for (auto &it_per_id : feature)
    {
        ...
        // it_per_id.valid_project_num = 0;
        // for (auto &it_per_frame : it_per_id.feature_per_frame) {
        //     it_per_frame.valid_project_edge = false;
        // }
        ...
        // Eigen::Vector3d twci = Ps[imu_i] + Rs[imu_i] * tic[0];
        // Eigen::Matrix3d Rwci = Rs[imu_i] * ric[0];
        ...
        // std::vector<Eigen::Vector2d> reprojection_error;

        // int feature_id = 0;
        // for (imu_j = it_per_id.start_frame + 1; imu_j < it_per_id.start_frame + (int)it_per_id.feature_per_frame.size(); imu_j++) {
            
        //     Eigen::Vector3d twcj = Ps[imu_j] + Rs[imu_j] * tic[0];
        //     Eigen::Matrix3d Rwcj = Rs[imu_j] * ric[0];
            
        //     Eigen::Matrix3d Rcwj = Rwcj.transpose();
        //     Eigen::Vector3d tcwj = -Rcwj * twcj;

        //     Eigen::Vector3d pts_j = Rcwj * pts_w + tcwj;
        //     pts_j /= pts_j.z();
        //     Eigen::Vector2d err = (pts_j - it_per_id.feature_per_frame[feature_id].point).head<2>();

        //     if(err.norm() < 10.0 / FOCAL_LENGTH) {
        //         it_per_id.feature_per_frame[feature_id].valid_project_edge = true;
        //         it_per_id.valid_project_num++;
        //     }
        //     reprojection_error.push_back(err);
        //     // std::cout << "pts_j: " << pts_j.transpose() << " uv: " << it_per_id.feature_per_frame[feature_id].point.transpose() << std::endl;
        //     feature_id++;
        // }

        // std::cout << "[triangulate] triangulation error: ";
        // for(size_t i = 0; i < reprojection_error.size(); i++) {
        //     double err = reprojection_error[i].norm();
        //     std::cout << err << " ";
        // }
        // std::cout << " valid num: " << it_per_id.valid_project_num << std::endl;
        // std::cout << std::endl;
    }
}
...
void FeatureManager::triangulateWithDepth(Vector3d Ps[], Vector3d tic[], Matrix3d ric[])
{
    for (auto &it_per_id : feature)
    {
        ...
        for (int i=0; i < (int)it_per_id.feature_per_frame.size(); i++)
        {
			...
			double depth_threshold = 6; //for handheld and wheeled application. Since d435i <3 is quiet acc
			//double depth_threshold = 10; //for tracked application, since IMU quite noisy in this scene             
			if (it_per_id.feature_per_frame[i].measure_depth < 0.05 || it_per_id.feature_per_frame[i].measure_depth >depth_threshold) 
                continue;
            Eigen::Vector3d point0(it_per_id.feature_per_frame[i].point * it_per_id.feature_per_frame[i].measure_depth);
            ...
            for (int j=0; j<(int)it_per_id.feature_per_frame.size(); j++)
            {
                ...
                if (residual.norm() < 10.0 / FOCAL_LENGTH) {//this can also be adjust to improve performance
