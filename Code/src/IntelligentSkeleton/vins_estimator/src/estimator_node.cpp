...
// int threshold = 3500;    //8.31 测试改
// int threshold = 2000;    //8.31 测试改
int threshold_hs ;   //-700; //350;       // 保持全局声明，但不初始化， 从json里面加载 10.9
...
void hsCallback(const std_msgs::UInt16MultiArray::ConstPtr &msg)   
{
    ...
    {   
        ...
        all_press_R =allhs-bias_R*600;
        // 更新 right_foot 的状态
        // if (tf1 >= 4200 || tf2 >= 4200 || hs1 >= 4200 || hs2 >= 4200)
        if ( all_press_R > threshold_hs  )   //21800 //21000
        {
            right_foot = 10;  // 10在地面
        }
        else
...
//新鞋垫对应的  hsCallback    LFC  10.25  版本1

// void hsCallback(const std_msgs::Float32MultiArray::ConstPtr &msg)
// {
//     // 数据约定（与发布端保持一致）：
//     // msg->data = {v0, v1, v2, sum, frame_id, side}
//     if (msg->data.size() < 6) return;

//     float sum = msg->data[3];
//     float side = msg->data[5]; // 1=左, 2=右
...
//         if (left_foot_history.size() > 3) left_foot_history.pop_front();
//         if (left_foot_history.size() == 3 &&
//             left_foot_history[0] == 20 &&
//             left_foot_history[1] == 20 &&
//             left_foot_history[2] == 20 &&
//             left_foot == 10) {
...
//     } else if (static_cast<int>(side) == 2) {
//         // 右脚
//         if (sum > threshold_hs) {
//             right_foot = 10; // 在地面
//         } else {
...
//         if (right_foot_history.size() > 3) right_foot_history.pop_front();
//         if (right_foot_history.size() == 3 &&
//             right_foot_history[0] == 20 &&
//             right_foot_history[1] == 20 &&
//             right_foot_history[2] == 20 &&
//             right_foot == 10) {
//             heel_strike_detected = true;
...
// 注意:回调函数类型改为Float32MultiArray
void hsCallback(const std_msgs::Float32MultiArray::ConstPtr &msg)   
{
    // 检查数据长度是否正确
    if (msg->data.size() != 7)
    {
        ROS_WARN("Invalid insole data size: %zu, expected 7", msg->data.size());
        return;
    }
    
    // I0, I1, I2 是左脚的三个传感器
    float left_sum = msg->data[0] + msg->data[1] + msg->data[2];
    
    // I3, I4, I5 是右脚的三个传感器
    float right_sum = msg->data[3] + msg->data[4] + msg->data[5];
    
    // I6 是帧数(可用于调试)
    float frame_number = msg->data[6];
    
    // 阈值150 (你说要在json里改,这里用变量threshold_hs)
    // threshold_hs 应该在程序开头从参数或配置文件读取
    
    // 处理左脚
    all_press_L = left_sum;
    
    if (all_press_L > threshold_hs)  // 超过150表示落地
    {
        left_foot = 10;  // 10在地面
    }
    else
    ...
    // 处理右脚
    all_press_R = right_sum;
    
    if (all_press_R > threshold_hs)  // 超过150表示落地
    {
        right_foot = 10;  // 10在地面
    }
    else
...
void FrontendLoop()
{
    while (true)
    {
        ...
        for (auto &measurement : measurements)
        {
            ...
            // cur_depth_img.copyTo(cur_depth_raw);
...
void BackendLoop()
{
    while (1)
    {
        ...
        // surf_feature—before               //原始平面处理
        // ImagePlaneFeature surf_feature_raw = extractSurfaces(depth_rt_ptr -> image, K, Twc_kf, 3.0, 0.0);
        // ImagePlaneFeature surf_feature = extractSurfaces(cur_depth_raw, K, Twc_kf, 3.0, 0.0);
...
struct sendDataStorage
{
    ...
    float position;  
    // 默认构造函数，初始化为零
    sendDataStorage() : newtimestamp(0), stairheight(0.00f), t2mdist(0.00f), velocity(0.00f), manheight(0.00f) ,position(0.00f) {}
};    // ZCX:1720 
        ...
        if (mapper->surface != NULL && !(mapper->surface->empty()))
        {
           ...
           // === 修改点1：替换原锁，复制前台数据 ===
            ...
            else
            {
                ...
                // s_height = vec_pair_min_dist_mean_z[0].second - vec_pair_min_dist_mean_z[1].second;   // 尝试 把abs 去掉  看上下楼梯的情况
                // next_dist = vec_pair_min_dist_mean_z[0].first - vec_pair_min_dist_mean_z[1].first;
...
int main(int argc, char **argv)
{
    ...
    // lfc 10.9 从json配置文件加载阈值和身高
    if (scene_graph_cfg.contains("threshold_hs")) {
        threshold_hs = scene_graph_cfg["threshold_hs"];
        ROS_INFO("Loaded threshold from config: %d", threshold_hs);
    } else {
        threshold_hs = 350;  // 默认值
        ROS_WARN("Threshold not found in config, using default: %d", threshold_hs);
    }

    if (scene_graph_cfg.contains("man_height")) {
        globalData.manheight = scene_graph_cfg["man_height"];
        ROS_INFO("Loaded man height from config: %.2f mm", globalData.manheight);
    } else {
        globalData.manheight = 1880.0;  // 默认值
        ROS_WARN("Man height not found in config, using default: %.2f mm", globalData.manheight);
    }
    
    bias_L = scene_graph_cfg["bias_L"];
    bias_R = scene_graph_cfg["bias_R"];
    ...
    std::thread backend(BackendLoop);      // 2     考虑在这个里面，直接将这个 GetGroundPos  ，输入 需要调整 ，调整成 矩阵形式  ，后面三个变量 都是 输出。
