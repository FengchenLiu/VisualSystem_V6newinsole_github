...
void new_sequence()
{
    ...
    pose_graph.posegraph_visualization->reset();
    pose_graph.publish();
    m_buf.lock();
...
void process()
{
    ...
    while (true)
    {
        ...
        if (pose_msg != NULL)
        {
            ...
            cv::Mat colormap = ptr->image;
            cv::Mat depthmap = depth_ptr->image;
            // build keyframe
            ...
            if((T - last_t).norm() > SKIP_DIS)
            {
                ...
                for (int i = L_BOUNDARY; i < COL - R_BOUNDARY; i += PCL_DIST)
                {
                    for (int j = U_BOUNDARY; j < ROW - D_BOUNDARY; j += PCL_DIST)
                    {
                        ...
                        float depth_val = ((float)depthmap.at<unsigned short>(j, i)) / 1000.0;
                ...
                KeyFrame* keyframe = new KeyFrame(pose_msg->header.stamp.toSec(), frame_index, T, R, colormap, point_3d_depth,
                                   point_3d, point_2d_uv, point_2d_normal, point_id, sequence);
                m_process.lock();
                start_flag = 1;
                ROS_INFO("addKeyFrame");
                depthmap.convertTo(depthmap, CV_32F, 0.001f, 0.0);
                pose_graph.addKeyFrame(keyframe, colormap, depthmap, 1);
                m_process.unlock();
                frame_index++;
                last_t = T;
            }
...
void command()
{
    ...
    while(1)
    {
        ...
        {
            TicToc t_filter;
            pose_graph.pclFilter(false);
            printf("pclFilter time: %f", t_filter.toc());
        }
        if (c == 'd')
        {
            TicToc t_pcdfile;
            pose_graph.save_cloud->width = pose_graph.save_cloud->points.size();
            pose_graph.save_cloud->height = 1;
	        pcl::io::savePCDFileASCII("/home/riger/pcd_file_"+to_string(frame_index)+"keyframes.pcd", *(pose_graph.save_cloud));
...
int main(int argc, char **argv)
{
    ...
    ROS_INFO("Enable LOOP_CLOSURE");
    if (LOOP_CLOSURE)
    {
        ROS_INFO("load fusion param");
        ...
        ROS_INFO("loadFusionParam");
        pose_graph.loadFusionParam(ROW, COL, FX, FY, CX, CY, MAX_DEPTH, MIN_DEPTH);


        //OctreePointCloudDensity has no ::Ptr
		pose_graph.octree = new pcl::octree::OctreePointCloudDensity<pcl::PointXYZ>(RESOLUTION);
	    pose_graph.cloud = pcl::PointCloud<pcl::PointXYZ>::Ptr(new pcl::PointCloud<pcl::PointXYZ>());
        pose_graph.save_cloud = pcl::PointCloud<pcl::PointXYZ>::Ptr(new pcl::PointCloud<pcl::PointXYZ>());
		pose_graph.octree->setInputCloud(pose_graph.cloud);
        pose_graph.octree->addPointsFromInputCloud();
		// in pcl 1.8.0+, need to set bbox (isVoxelOccupiedAtPoint will check bbox)
		pose_graph.octree->defineBoundingBox(-100, -100, -100, 100, 100, 100);
