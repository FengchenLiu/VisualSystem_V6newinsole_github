...
// Compute surface points from TSDF voxel grid and save points to point cloud file
void SaveVoxelGrid2SurfacePointCloud2(const std::string &file_name, Voxel* voxels, int voxel_grid_dim_x, int voxel_grid_dim_y, int voxel_grid_dim_z,
                                     float voxel_size, float voxel_grid_origin_x, float voxel_grid_origin_y, float voxel_grid_origin_z,
                                     float tsdf_thresh, float weight_thresh) {

  // Count tota// Load an M x N matrix from a text file (numbers delimited by spaces/tabs)
...
s(voxels[i].tsdf) < tsdf_thresh && voxels[i].weight > weight_thresh)
      num_pts++;
  ...
  for (int i = 0; i < voxel_grid_dim_x * voxel_grid_dim_y * voxel_grid_dim_z; i++) {
    ...
    // If TSDF value of voxel is less than some threshold, add voxel coordinates to point cloud
    if (std::abs(voxels[i].tsdf) < tsdf_thresh && voxels[i].weight > weight_thresh) {
