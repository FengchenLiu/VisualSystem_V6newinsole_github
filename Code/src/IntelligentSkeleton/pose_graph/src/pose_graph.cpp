...
void PoseGraph::loadFusionParam(int h, int w, double fx, double fy, double cx, double cy, double max_depth, double min_depth)
{
    cv::Mat K = (cv::Mat_<float>(3, 3) << fx, 0, cx, 0, fy, cy, 0, 0, 1);
    TSDFMapper::TSDFMappingOptions options;
    options.height = h;
    options.width = w;
    options.grid_dim_x = 512;
    options.grid_dim_y = 512;
    options.grid_dim_z = 512;
    options.voxel_size = 0.01f;
    options.K = K;
    options.max_depth = max_depth;
    options.min_depth = min_depth;
    tsdf_mapper = new TSDFMapper(options);
}
...
void PoseGraph::addKeyFrame(KeyFrame* cur_kf, cv::Mat& colormap, cv::Mat& depthmap, bool flag_detect_loop)
{
    //shift to base frame
    Vector3d vio_P_cur;
    Matrix3d vio_R_cur;
    ...
    Eigen::Matrix4d Twi = Eigen::Matrix4d::Identity(), Tic = Eigen::Matrix4d::Identity();
    Twi.block<3, 3>(0, 0) = R;
    Twi.block<3, 1>(0, 3) = P;
    Tic.block<3, 3>(0, 0) = qi_d;
    Tic.block<3, 1>(0, 3) = ti_d;
    Eigen::Matrix4d Twc = Twi * Tic;
